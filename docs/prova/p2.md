...
### Questão 4: Considerando que as variáveis i, j e k são, respectivamente, manipuladas através dos registradores te, tl e t2, implemente os seguintes fragmentos de códigos de linguagem C na linguagem de montagem da arquitetura RISCV

## a) `if (i < 87) j = j + 33; else j = j - k;`

### Código em RISC-V (fragmento)

```asm
    # if (i < 87) j = j + 33; else j = j - k;
    # i -> t0, j -> t1, k -> t2

    li   t3, 87          # t3 = 87
    bge  t0, t3, else_a  # se i >= 87, vai para o else

    addi t1, t1, 33      # then: j = j + 33
    j    end_a           # pula o else depois de executar o then

else_a:
    sub  t1, t1, t2      # else: j = j - k
...
loop_b:
    addi t0, t2, 29      # i = k + 29
    slli t2, t2, 1       # k = 2 * k  (desloca 1 bit à esquerda)
    addi t1, t1, 1       # j++

    li   t3, 100         # t3 = 100
    ble  t1, t3, loop_b  # enquanto (j <= 100) repete o loop
```


### Questão 5: Analise o seguinte fragmento de código e calcule o valor em t2 ao alcançar o label Fim. Atenda ao comentário indicado no começo da seção de dados para sua análise: 
